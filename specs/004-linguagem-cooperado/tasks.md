...
- [x] **T001** `npm run build` **antes de qualquer edição** e guardar
      `dist/sitemap-0.xml` fora de `dist/`. É a linha de base do SC-004 — sem
      ela não há como provar que nenhuma URL mudou.
- [x] **T002** `test/vocabulario.test.mjs` — varre `src/**` com `node:fs`, três
...
- [x] **T003** Rodar `npm test` e **ver a lista das 57 ocorrências**. É o mapa
      de trabalho das fases seguintes; teste que nasce verde não guarda nada.
- [x] **T004** Preencher a lista de exceções com as **7 entradas aprovadas**,
...
- [x] **T005** `src/data/solucoes.ts` — `descricao` (é a meta description da
...
- [x] **T006** `src/pages/index.astro` — o arquivo mais denso da feature:
...
- [x] **T007** `src/pages/parceiro.astro` — FAQ do critério de indicação: a
...
- [x] **T008** [P] `src/components/Fatura.astro` — passos "Você se associa" e
      "unidade associada" (o carimbo do desenho da fatura), comentário do
      propósito da unidade.
- [x] **T009** [P] `src/components/Footer.astro` (ressalva + "termo de adesão"),
...
- [x] **T010** `src/pages/termos.astro` — abertura, declaração de papéis,
...
- [x] **T011** `src/pages/privacidade.astro` — trocar apenas a linha de prosa
...
- [x] **T012** `reduzir-conta-equatorial-sem-placa-solar.md` — artigo pilar:
      regra dos R$ 250 (ponto 7 de 7) e vocabulário no corpo, `titulo` e
      `descricao`.
- [x] **T013** [P] `fio-b-60-por-cento-2026-conta-equatorial-goias.md` — uma
      ocorrência ("associando-se").
- [x] **T014** `cancelar-associacao-cooperativa-energia-goias.md` — 11
...
- [x] **T015** `src/pages/llms.txt.ts` — descrição do papel, "termo de adesão",
      regra dos R$ 250 (ponto 6) e prazo em dois tempos. Conferir que o
      percentual anunciado é o mesmo da home (FR-011).
- [x] **T016** `src/layouts/Base.astro` — `description` do JSON-LD e o
...
- [x] **T017** `npm test` verde, com a guarda do FR-018 inclusa (SC-005).
      Saída vista, não presumida.
- [x] **T018** `grep -rniE "associa" src/` — o resultado deve ser **exatamente**
      as 7 exceções da T004, nada além (SC-001).
- [x] **T019** `npm run build` e `diff` do `sitemap-0.xml` contra a linha de
      base da T001. Zero diferença, zero 301 novo (SC-004, FR-017).
- [x] **T020** Ler os **7 trechos da regra lado a lado** e confirmar que
...
- [x] **T021** Conferir promessa única de prazo e de percentual em todo o site,
